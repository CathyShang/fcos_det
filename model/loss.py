...
class GenTargets(nn.Module):
    ...
    def forward(self,inputs):
        ...
        for level in range(len(cls_logits)):
            ...
            # todo by shangzq
            # anchor_points_all_level.append(level_targets[3])
            # if level == 0:
            #     anchor_points=level_targets[3]

        return torch.cat(cls_targets_all_level,dim=1),torch.cat(cnt_targets_all_level,dim=1),torch.cat(reg_targets_all_level,dim=1) #, anchor_points #torch.cat(anchor_points_all_level,dim=1)
...
def compute_cls_loss(preds,targets,mask):
    ...
    for batch_index in range(batch_size):
        ...
        if cfg.focal_loss:
            ...
            loss.append(bce_loss_from_logits(pred_pos, target_pos).view(1))
...
def iou_loss(preds,targets):
    ...
    # 求了个log
    loss=-iou.clamp(min=1e-6).log() #这里的log 是 ln 自然对数
    return loss.sum()
...
# todo by 2022-04-07  bca
def bce_loss_from_logits(preds,targets, alpha=0.25):
    preds = preds.sigmoid()
...
class LOSS(nn.Module):
    ...
    def forward(self,inputs):
        ...
        # cls_targets,cnt_targets,reg_targets, anps = targets
        cls_logits,cnt_logits,reg_preds = preds
        ...
        # reg_loss=compute_reg_loss(reg_preds,reg_targets,mask_pos, anps).mean()

        # todo by shangzq 多个选择
...
if __name__=="__main__":
    ...
    # test = LOSS()
    import torch
    # # pred [batch_size,class_num,_h,_w]
    # cls_logits = torch.randn((1,2,1,7,7))
    # cls_targets = torch.randn((2,49,1))
    # cnt_targets = torch.randn((2,49,1))
    # mask_pos = (cnt_targets > -1).squeeze(dim=-1)  # [batch_size,sum(_h*_w)]
    # # 每个批次的损失，最后求解均值
    # cls_loss = compute_cls_loss(cls_logits, cls_targets, mask_pos).mean()
    ...
    # 学习iou loss 计算
    # preds = torch.Tensor([[10,10,20,20]])
    # targets = torch.Tensor([[5,15,15,25]])
